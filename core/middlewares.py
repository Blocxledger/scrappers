...
class CurlCffiDownloaderMiddleware:
    """Downloader middleware using curl_cffi"""
    ...
    def from_crawler(cls, crawler):
        ...
        impersonate = crawler.settings.get(
            "CURL_CFFI_IMPERSONATE", "chrome"
        )

        mw = cls(timeout=timeout, impersonate=impersonate)
        crawler.signals.connect(mw.spider_opened, signal=signals.spider_opened)
        return mw

    def spider_opened(self, spider):
        spider.logger.info("Using curl_cffi downloader middleware")

    def process_request(self, request, spider):
        """
        Intercept Scrapy request and fetch it using curl_cffi
        """
